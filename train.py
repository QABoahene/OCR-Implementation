...
import transform
from dataset import IDD_Benchmark, collate_data
...
def data_sampler(IDD_Benchmark, shuffle, distributed):
    if distributed:
        return DistributedSampler(IDD_Benchmark, shuffle=shuffle)

    if shuffle:
        return sampler.RandomSampler(IDD_Benchmark)

    else:
        return sampler.SequentialSampler(IDD_Benchmark)
...
if __name__ == '__main__':
    ...
    train_set = IDD_Benchmark(args.path, 'train', train_trans)
    valid_set = IDD_Benchmark(args.path, 'valid', valid_trans)
